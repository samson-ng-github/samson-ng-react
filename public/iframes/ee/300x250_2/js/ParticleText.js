(function(window) {
	...
	ParticleText = function(_fontClass,_fontSize,_lines,_align,_lineHeight) {
			...
			for (var j=0; j<_text.length; j++) {
				...
				if (char != " ") {
					...
					for (var k=0; k<LP.length; k++) {
						...
						var thisPoint = LP[k];
						var newPoint = new createjs.Point((thisPoint[0]*_fontSize/30)+_letterPos, (thisPoint[1]*_fontSize/30)+_vertOffset);
						this.points.push(newPoint);
						this.letterPoints[j].push(newPoint);

					}

					_letterPos += _fontClass.getSpacing(_text.charAt(j),_text.charAt(j+1))*_fontSize/30;

				} else {

					_letterPos += 8*_fontSize/30;
				}
			}
		...
		function getPoints(char) {
			...
			if ("ABCEDFGHIJKLMNOPQRSTUVWXYZx".indexOf(char) != -1)
			{
			...
			else
			{
				...
				switch (char)
				{
				   ...
				   case "*":
					...
					case "i" :
						pts = _fontClass.$i.slice(0);
						break;
					case "h" :
						pts = _fontClass.$h.slice(0);
						break;
					case "o" :
						pts = _fontClass.$o.slice(0);
						break;
					case "n":
						pts = _fontClass.$n.slice(0);
						break;
				   case "e":
				   		pts = _fontClass.$e.slice(0);
						break;
				}
			}
