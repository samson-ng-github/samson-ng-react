...
var status, button_status = "buckle";	
...
function load_data(button_status) {
	...
	var model_texture_path, model_path, model_rotat_adjust, model_trans_adjust, model_scale, model_rotation_speed;
	...
	hotspot_position = [
		...
		new THREE.Vector3 (-0.030,  0.063, -0.428),
		new THREE.Vector3 (-0.108,  0.080, -0.021),
		new THREE.Vector3 (-0.360, -0.117,  0.483)
	];
	...
	init(model_texture_path, model_path, model_rotat_adjust, model_trans_adjust, model_scale, model_rotation_speed, hotspot_path, hotspot_text, hotspot_position, bg_text_path, bg_text_path_w, bg_text_path_h, bg_color);
	animate();
...
function init(model_texture_path, model_path, model_rotat_adjust, model_trans_adjust, model_scale, model_rotation_speed, hotspot_path, hotspot_text, hotspot_position, bg_text_path, bg_text_path_w, bg_text_path_h, bg_color) {
	...
	for ( var a = 0; a < hotspot_text.length; a ++ ) {
		...
		var material = new THREE.SpriteMaterial( { map: sprite_map, color: 0xffffff, fog: true } );
		...
		sprite.position.set( hotspot_position[a].x, hotspot_position[a].y, hotspot_position[a].z );
		...
		sprite.index = a;
		sprite.angle = Math.atan2(sprite.position.z, sprite.position.x)*180/Math.PI;
		...
		object.position.set( sprite.position.x, sprite.position.y, sprite.position.z );
	...
	loader.load( model_path, function ( object ) {
		...
		object.traverse( function ( child ) {
			...
			if ( child instanceof THREE.Mesh ) {child.material.map = texture; child.material.side = THREE.DoubleSide;}

		} );

		object.scale.set(model_scale,model_scale,model_scale);
		object.rotation.set(model_rotat_adjust.x, model_rotat_adjust.y, model_rotat_adjust.z);
		object.position.set(model_trans_adjust.x*model_scale, model_trans_adjust.y*model_scale, model_trans_adjust.z*model_scale);
		group.add( object );
...
function preloader() {
	...
	THREE.ImageUtils.loadTexture( "assets/textures/scale.png", undefined, function (texture) {
	...
	THREE.ImageUtils.loadTexture( "assets/textures/dial.png", undefined, function (texture) {
...
function render() {
	...
	if (status == "preloading_model") {
		...
		if (dial.material.rotation == real_progress && real_progress > -Math.PI/2) dial.material.rotation -= 0.1;
	...
	if (camera_tracks) {
		...
		for (var i = 0; i < tags.length; i ++)
			if (tags[i].element.style.opacity > 0.05) tags[i].element.style.opacity -= 0.05;
			else if (tags[i].element.style.opacity <= 0.05) tags[i].element.style.opacity = 0;
	...
	if (clickable) {
		...
		if ((( targetRotation - group.rotation.y ) * 0.05 < 0.01) && keep_moving) camera_tracks = true;
	
	}
	
	for (var i = 0; i < tags.length; i ++) tags[i].rotation.y = Math.PI / 2 - camera_angle - group.rotation.y;
	camera.lookAt( new THREE.Vector3 (scene.position.x, camera.position.y, scene.position.z) );
